        ...
        return (
          ...
          >
            ...
            <div className="flex items-start justify-between px-2 pt-2 relative z-10">
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  <span className={`text-base md:text-lg font-semibold leading-tight ${isHighlight ? "text-white" : "text-gray-900"}`}
                    style={{
                      fontFamily: 'Wix Madefor Text',
                      fontWeight: 500,
                      fontStyle: 'Medium',
                  ...
                  >{task.person}</span>
                  <span className={`text-xs  ${isHighlight ? "text-orange-100" : "text-[#7B8FA1]"}`}>{task.title}</span>
                </div>
              </div>
            ...
            <div className="flex items-center gap-4 px-6 pt-6 pb-2 relative z-10">
              <div className={`w-10 h-10   lg:w-14 lg:h-14 flex items-center justify-center rounded-full border ${isHighlight ? "border-orange-200 bg-orange-100/30" : "border-gray-200 bg-white"}`}>
                {task.name === "Google Meet Call" ? (
                  <img src="/src/assets/gmeet.png" alt="Google Meet" className="h-5 w-5 lg:w-7 lg:h-7" />
                ) : (
                  <FileText className={`w-7 h-7 ${isHighlight ? "text-white" : "text-gray-400"}`} />
                )}
              </div>
              <div className="flex flex-col">
                <h3 className={`text-base lg:text-2xl font-bold mb-1 ${isHighlight ? "text-white" : "text-gray-900"}`}>{task.name}</h3>
                {task.date && task.time && (
            ...
            <div className="flex items-center justify-between md:gap-1 px-4 py-2 relative z-10">
              <div className={`flex items-center gap-3 rounded-full border px-2 py-1 lg:px-4 lg:py-2  ${isHighlight ? "bg-orange-100/30 border-orange-200" : "bg-white border-gray-200"}`}>
                <img src={task.avatar} alt={task.person} className="w-8 h-8 rounded-full border-2 border-white object-cover" />
                <span className={`text-sm  font-medium ${isHighlight ? "text-white" : "text-gray-900"}`}>{task.status}</span>
                <ArrowDown className={`w-4 h-4 md:hidden lg:flex ${isHighlight ? "text-white" : "text-gray-400"}`} />
              </div>
              <div className="flex gap-2">
                <button className={`w-10 h-10 flex items-center justify-center rounded-full ${isHighlight ? "bg-white/20 border border-white text-white" : "bg-gray-100 border border-gray-200 text-gray-500"}`}>
                ...
                {isHighlight && (
                  <button className="w-10 h-10 flex items-center justify-center rounded-full bg-[#CB122D] border border-white text-white"><Video className="w-7 h-7" /></button>
                )}
              </div>
            </div>
          </div>
